...
public class BankLeaderboard extends Leaderboard {
    ...
    public void update() {
        ZGangs.getThreadPool().execute(() -> {
            ...
            FindIterable<Document> gangs = ZGangs.getDatabase().getGangCollection().find()
                    .sort(new BasicDBObject("bank", -1)).limit(21);
            ...
            leaderboard.removeIf(entry -> entry.getLeaderboardType() != LeaderboardType.BANK);
