...
public class GuardKillsLeaderboard extends Leaderboard {
    ...
    public void update() {
        ZGangs.getThreadPool().execute(() -> {
            ...
            FindIterable<Document> gangs = ZGangs.getDatabase().getGangCollection().find()
                    .sort(new BasicDBObject("gangStats.total_guard_kills", -1)).limit(21);
            try (MongoCursor<Document> gangIterator = gangs.iterator()) {
                while (gangIterator.hasNext()) {
                    ...
                    GangLeaderboardEntry gangEntry = new GangLeaderboardEntry(
                            gang.getUUID(),
                            gang.getOwnerUUID(),
                            LeaderboardType.GUARD_KILLS,
                            gang.getGangStats().getTotal_guard_kills()
                    );
                    leaderboard.add(gangEntry);
                }
            }

            leaderboard.removeIf(entry -> entry.getLeaderboardType() != LeaderboardType.GUARD_KILLS);
